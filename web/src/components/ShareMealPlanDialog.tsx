...
interface ShareMealPlanDialogProps {
  ...
  planName: string;
  /**
   * Current explicit-share list pulled off the plan doc. Pass the
   * latest snapshot in so changes outside the dialog (e.g. an
   * unshare from another tab) reflect when re-opened.
   */
...
/**
 * Per-meal-plan share modal. Mirror of ShareDialog (the recipe
 * version) — owner enters a family member's email, we resolve it to a
 * uid server-side via shareMealPlan, append it to the plan's
 * `sharedWith`, and denormalize {uid, email} into `sharedWithDetails`
 * so the UI can show who has access without re-hitting Auth.
 *
 * Shares are read-only on the grantee side — they can view the plan
 * but can't edit it (firestore.rules keeps `mealPlans/{id}` writes
 * owner-only). The copy makes that explicit so the owner doesn't
 * expect collaborative editing.
 *
 * Built on the same native `<dialog>` pattern as ShareDialog —
 * focus trap, Esc, inert-outside, and backdrop click all come free.
 */
export function ShareMealPlanDialog({
  ...
  async function handleAdd() {
    ...
    setError(null);
    try {
      const result = await callShareMealPlan({
        planId,
      ...
      console.error("shareMealPlan:", err);
  ...
  async function handleRemove(granteeUid: string) {
    ...
    setError(null);
    try {
      await callUnshareMealPlan({ planId, granteeUid });
      onChange(sharedWithDetails.filter((d) => d.uid !== granteeUid));
    } catch (err) {
      console.error("unshareMealPlan:", err);
    ...
    >
      <div className="bg-white rounded-xl shadow-lg p-6 max-w-[480px] w-[92vw]">
        ...
        <p className="font-sans text-sm text-ink-700 m-0 mb-5">
          <span className="font-display italic">{planName}</span> — anyone you
          add can see this plan but not edit or delete it.
        </p>
        ...
        {sharedWithDetails.length > 0 ? (
          ...
          <p className="font-sans text-sm text-ink-500 mt-5 m-0">
            No one else has access to this plan yet. To share every plan
            (and every recipe) with a family member, use{" "}
