...
const SYSTEM_PROMPT = `You are a recipe extraction assistant. You receive a single image of a recipe — a photograph of a cookbook page, a magazine clipping, a handwritten note card, a screenshot, or a printout. Use the extract_recipe tool to return structured data. Always use the tool — do not return free-form text.

Read what's actually in the image, even if the handwriting is messy, the photo is at an angle, or the lighting is uneven. Use context to infer obvious things (a "Cake" heading above a list followed by a "Frosting" heading above another list means two ingredient sections).

Field guide:
- title (required) — the recipe's name as printed or written. If the image has no title (e.g. it's just a list of ingredients), name it after the dish the ingredients describe.
...
- tags — lowercase kebab-case attributes like "gluten-free", "vegetarian", "dairy-free" — only if clearly indicated in the image (e.g. a "Vegetarian" label, a leaf icon, an explicit note).
- source — if the image clearly shows a book title / author / page number (cookbook page, magazine masthead), emit source as {type: "book", title, author, page}. Omit otherwise — the server will not override source for photo imports, unlike the URL importer.
- photoUrl — leave empty. The image the user just shared IS the recipe; we'll attach it on the client.
...
If the image clearly does NOT contain a recipe (a sunset photo, a meme, a screenshot of a tweet), return title: "Not a recipe" with a single ingredients section heading: null containing one item describing what the image actually shows. The client will surface this so the user knows the OCR ran but found nothing usable.`;
...
export const importFromImage = onCall(
  ...
  async (request) => {
    ...
    if (!mimeType || !ALLOWED_MIME_TYPES.has(mimeType)) {
      throw new HttpsError(
        "invalid-argument",
        `Unsupported image type. Use one of: ${Array.from(ALLOWED_MIME_TYPES).join(", ")}.`,
      );
    }
    if (imageBase64.length > MAX_BASE64_BYTES) {
      throw new HttpsError(
        "invalid-argument",
        `Image is too large (${(imageBase64.length / 1024 / 1024).toFixed(1)} MB after base64 encoding). Resize to under 5 MB.`,
      );
    }
        ...
        tools: [
          {
            name: "extract_recipe",
            description:
              "Return the structured recipe extracted from the provided image.",
        ...
        messages: [
          {
            ...
            content: [
              ...
              {
                type: "text",
                text: "Extract the recipe from this image using the extract_recipe tool.",
              },
            ],
          },
        ],
